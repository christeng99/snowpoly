...
class MarketWebSocket:
    ...
    def __init__(
        ...
        # Per connection: first live subscribe uses {"type": "MARKET"}; later adds use
        # {"operation": "subscribe"} (see Polymarket CLOB WS protocol).
        self._market_subscription_sent: bool = False
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._market_subscription_sent = False
            if self._on_connect:
                self._on_connect()
            return True
    ...
    async def _send_assets_operation(self, asset_ids: List[str], operation: str) -> bool:
        """Send subscribe/unsubscribe operation (post–initial MARKET only)."""
        if not asset_ids:
            return True
        msg: Dict[str, Any] = {"assets_ids": asset_ids, "operation": operation}
        try:
            await self._ws.send(json.dumps(msg))
            logger.info("WS %s for %d assets", operation, len(asset_ids))
            return True
        except Exception as e:
    ...
    async def subscribe(self, asset_ids: List[str], replace: bool = False) -> bool:
        ...
        The first subscription after each connection sends ``{"type": "MARKET"}`` only.
        Later updates send ``operation: "unsubscribe"`` for dropped tokens and
        ``operation: "subscribe"`` for new ones (including when ``replace=True``).

        Args:
            asset_ids: List of token IDs to subscribe to (may be empty when replace=True
                to drop all subscriptions while connected).
            replace: If True, replace tracked set with asset_ids (clears orderbooks first).

        Returns:
            True if subscription sent successfully
        """
        if not asset_ids and not replace:
            return False

        old_subscribed: Set[str] = set(self._subscribed_assets)
        ...
        logger.info(
            "subscribe() called with %d assets (replace=%s), is_connected=%s",
            len(asset_ids),
            replace,
            self.is_connected,
        )
        ...
        if not self.is_connected:
            logger.info("Not connected yet; subscription will apply on next connect")
            return True

        # Exactly one MARKET message per WebSocket connection (initial snapshot request).
        if not self._market_subscription_sent:
            if not asset_ids:
                return True
            try:
                ...
                logger.info(f"Sending MARKET subscribe: {msg_json[:200]}")
                await self._ws.send(msg_json)
                self._market_subscription_sent = True
                logger.info("MARKET subscription sent (%d assets)", len(asset_ids))
                return True
            except Exception as e:
        ...
        if replace:
            ...
            to_unsub = [a for a in old_subscribed if a not in new_set]
            to_sub = list(
                dict.fromkeys(a for a in asset_ids if a not in old_subscribed)
            )
        else:
            to_unsub = []
            to_sub = list(
        ...
        ok = True
        if to_unsub:
            ok = await self._send_assets_operation(to_unsub, "unsubscribe") and ok
        if to_sub:
            ok = await self._send_assets_operation(to_sub, "subscribe") and ok
        return ok
    ...
    async def subscribe_more(self, asset_ids: List[str]) -> bool:
        ...
        if not self._market_subscription_sent:
            try:
                ...
                self._market_subscription_sent = True
                logger.info(f"MARKET subscription sent (%d assets)", len(to_sub))
                return True
            except Exception as e:
                logger.error(f"Failed to subscribe: {e}")
                if self._on_error:
        ...
        return await self._send_assets_operation(to_sub, "subscribe")
    ...
    async def unsubscribe(self, asset_ids: List[str]) -> bool:
        ...
        return await self._send_assets_operation(list(asset_ids), "unsubscribe")
    ...
    async def _run_loop(self) -> None:
        ...
        while self._running and self.is_connected:
            try:
                ...
                logger.error(
                    "Failed to parse message: %s | raw=%r",
                    e,
                    message,
                )
            except Exception as e:
